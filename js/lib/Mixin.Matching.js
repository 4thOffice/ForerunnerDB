                    ...
                    for (resultIndex = 0; resultIndex < docArr.length; resultIndex++) {
                        ...
                        if (!joinRequire || (joinRequire && joinFindResults[0])) {
                            ...
                            if (resultKeyName === '$root') {
                                ...
                                // Allow nested '$as' names
                                var currentLevel = docArr[resultIndex];
                                var resultKeyNames = resultKeyName.split('.');
                                for (var i = 0; i < resultKeyNames.length; i++) {
                                    var levelName = resultKeyNames[i];

                                    // Add resoult to last branch
                                    if (i === resultKeyNames.length - 1) {
                                        currentLevel[levelName] = joinMulti === false ? joinFindResults[0] : joinFindResults;
                                        continue;
                                    }
